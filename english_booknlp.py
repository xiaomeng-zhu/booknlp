...
class EnglishBookNLP:
	...
	def __init__(self, model_params): # when creating new instance of EnglishBookNLP, give it model parameters (a dict)
		...
		with torch.no_grad(): # disabled gradient calculation
			...
			tagsetPath="booknlp/english/data/entity_cat.tagset"
			...
			if "referential_gender_hyperparameterFile" in model_params:
				...
				self.gender_hyperparameterFile = pkg_resources.resource_filename(__name__, "booknlp/english/data/gutenberg_prop_gender_terms.txt")
			...
			if self.doEntities:
				...
				aliasPath = pkg_resources.resource_filename(__name__, "booknlp/english/data/aliases.txt")
		...
		with torch.no_grad():
			...
			with open(filename) as file: # open the book txt file to be processed
				...
				if self.doEntities:
					...
					# need to check if a first person pronoun is in a quote
